...
export async function generateMetadata({ params: paramsPromise }: Args): Promise<Metadata> {
  ...
  return {
    title: atlasLocation
      ? `${atlasLocation.title} | Blog de Viagem | Página ${pageNumber}`
...
export async function generateStaticParams() {
  ...
  for (const atlasLocation of atlasLocations.docs) {
    const { totalDocs } = await payload.count({
      collection: 'posts',
      overrideAccess: false,
      where: {
        locations: { contains: atlasLocation.id },
        publishedAt: { less_than_equal: new Date().toISOString() },
      },
    })
    const totalPages = Math.ceil(totalDocs / 12)
    // DECISION: página 1 já é servida por /atlas/[slug] — gerar estaticamente
    // só a partir da página 2 evita duplicar a mesma rota em dois caminhos.
    for (let i = 2; i <= totalPages; i++) {
      params.push({ slug: atlasLocation.slug, pageNumber: String(i) })
    }
  }
